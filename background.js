...
function onDPTabCreated(tab, windowID) {
    ...
    function onGot(tab) {
        ...
        if (tab.url == "https://manage.bittitan.com" || tab.url.indexOf("manage.bittitan.com/delivery") != -1) { //the two URLs are the default redirection for IMPERSONATION, when URL is either one, account is impersonated, proceed to redirect
...
browser.runtime.onMessage.addListener(function (request, sender, sendResponse) {
    ...
    } else if (request.action == 'getTabImpersonationId') // ACTION 5
    {
        ...
        if (GLOBALTABIDMAP.get(sender.tab.id) == undefined) {// tab with no tagged ImpersonationID
